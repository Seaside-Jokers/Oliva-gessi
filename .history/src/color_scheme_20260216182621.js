...
function updateThemeColors() {
    ...
    for (const [key, value] of Object.entries(colors)) {
        // Converte camelCase in kebab-case (es: primaryContainer -> primary-container)
        const cssVarName = key.replace(/([A-Z])/g, '-$1').toLowerCase();
        root.style.setProperty(`colo-${cssVarName}`, value);
    }
}
...
function toggleLightDark() {
    ...
    const contrastLevel = currentColorScheme.includes('high-contrast') ? '-high-contrast' : 
                         currentColorScheme.includes('medium-contrast') ? '-medium-contrast' : '';
    
    const newScheme = isDark ? `light${contrastLevel}` : `dark${contrastLevel}`;
    changeColorScheme(newScheme);
}

function setColorScheme(scheme, contrastLevel = '') {
    // scheme può essere 'light' o 'dark'
    // contrastLevel può essere '', 'medium-contrast' o 'high-contrast'
...
window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', (e) => {
    ...
    // Mantieni il livello di contrasto attuale
    const contrastLevel = currentColorScheme.includes('high-contrast') ? 'high-contrast' : 
                         currentColorScheme.includes('medium-contrast') ? 'medium-contrast' : '';
...
// Fonts: https://ctrl-z.it/9-font-architettura-tavole-portfolio https://material-f/
// Design:oundation.github.io/material-theme-builder/